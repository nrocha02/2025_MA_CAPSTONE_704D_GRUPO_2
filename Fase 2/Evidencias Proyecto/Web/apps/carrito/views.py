...
def validar_rut(rut):
    ...
    dv_calculado = 'K' if dv_esperado == 10 else ('0' if dv_esperado == 11 else str(dv_esperado))
...
def iniciar_pago(request):
    ...
    try:
        ...
        logger.info(f"Iniciando transacción Transbank: buy_order={buy_order}, amount={amount}")
...
def confirmar_pago(request):
    ...
    try:
        ...
        if response['status'] == 'AUTHORIZED' and response['response_code'] == 0:
            ...
            if not checkout_session.cliente_persona_id and not checkout_session.cliente_empresa_id:
