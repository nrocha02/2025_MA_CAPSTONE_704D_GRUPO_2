...
from apps.ventas.models import (
    Producto, Categoria, Marca, Pedido, PedidoItem, Pago, 
    MovimientoStock, ClientePersona, ClienteEmpresa, DocumentoTributario
)
...
def admin_dashboard(request):
    ...
    productos_top = PedidoItem.objects.filter(
        pedido__fecha__gte=month_start,
        pedido__estado__in=['Procesando']
    ).values(
...
def reportes_productos(request):
    ...
    productos_con_ventas = productos_query.annotate(
        total_vendido=Coalesce(
            Sum('pedidoitem__cantidad',
                filter=Q(pedidoitem__pedido__fecha__date__gte=fecha_limite,
                         pedidoitem__pedido__estado__in=['Completado', 'Entregado'])), 0
        ),
        ingresos_generados=Coalesce(
            Sum(F('pedidoitem__cantidad') * F('pedidoitem__precio_unitario'),
                filter=Q(pedidoitem__pedido__fecha__date__gte=fecha_limite,
                         pedidoitem__pedido__estado__in=['Completado', 'Entregado'])), 0
        ),
        num_pedidos=Count('pedidoitem__pedido',
                         filter=Q(pedidoitem__pedido__fecha__date__gte=fecha_limite,
                                  pedidoitem__pedido__estado__in=['Completado', 'Entregado']),
                         distinct=True)
    )
    ...
    productos_activos = Producto.objects.filter(estado_producto='activo').count()
    productos_stock_bajo = Producto.objects.filter(stock__lt=10, estado_producto='activo').count()
    productos_sin_stock = Producto.objects.filter(stock=0, estado_producto='activo').count()
    ...
    productos_mas_vendidos = PedidoItem.objects.filter(
        pedido__estado__in=['Completado', 'Entregado']
    ).values(
        'producto__nombre',
        'producto__sku',
    ...
    categorias_stats = Categoria.objects.annotate(
        ...
        productos_stock_bajo=Count('producto', 
