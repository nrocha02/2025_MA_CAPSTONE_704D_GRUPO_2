    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=(
                    Q(nivel=1, categoria_padre__isnull=True)
                    | Q(nivel=2, categoria_padre__isnull=False)
                ),
                name="chk_categoria_principal",
            ),
            models.CheckConstraint(check=Q(nivel__in=[1, 2]), name="chk_nivel_valido"),
            models.UniqueConstraint(
                fields=["nombre", "categoria_padre"], name="uk_nombre_categoria"
            ),
...
class Direccion(models.Model):
    ...
    cliente_persona = models.ForeignKey(
        ClientePersona, on_delete=models.CASCADE, blank=True, null=True
    )
    cliente_empresa = models.ForeignKey(
        ClienteEmpresa, on_delete=models.CASCADE, blank=True, null=True
    )
        ...
        constraints = [
            models.CheckConstraint(
                check=(
                    ...
                    | (
                        Q(cliente_persona__isnull=True)
                        & Q(cliente_empresa__isnull=False)
                    )
                ),
...
class Producto(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=Q(precio__gte=0), name="producto_precio_check"
            ),
...
class Pedido(models.Model):
    ...
    cliente_invitado = models.ForeignKey(
        SesionInvitado, on_delete=models.CASCADE, blank=True, null=True
    )
    calle = models.CharField(max_length=50)
    ciudad = models.CharField(max_length=50)
    region = models.CharField(max_length=50)
    ...
    estado = models.CharField(
        max_length=50, default="Pendiente de pago"
    )  # enum estado_pedido
    total = models.IntegerField()
    notas = models.TextField(blank=True, null=True)
    tracking_codigo = models.CharField(max_length=50, blank=True, null=True)
    precio_envio = models.IntegerField(blank=True, null=True)
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=(
                    (
                        Q(cliente_persona__isnull=False)
                        & Q(cliente_empresa__isnull=True)
                        & Q(cliente_invitado__isnull=True)
                    )
                    | (
                        Q(cliente_persona__isnull=True)
                        & Q(cliente_empresa__isnull=False)
                    ...
                    | (
                        Q(cliente_persona__isnull=True)
                        & Q(cliente_empresa__isnull=True)
                        & Q(cliente_invitado__isnull=False)
                    )
                ),
        ...
        indexes = [
            ...
            models.Index(
                fields=["cliente_invitado"], name="idx_pedido_cliente_invitado"
            ),
...
class PedidoRegistro(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=~Q(estado_anterior=models.F("estado_actual")),
                name="chk_cambio_estado_pedido",
            )
        ]
...
class DocumentoTributario(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=Q(subtotal__gte=0), name="documento_tributario_subtotal_check"
            ),
            models.CheckConstraint(
                check=Q(total__gte=0), name="documento_tributario_total_check"
            ),
            models.CheckConstraint(
                check=(
                    ~Q(tipo_documento="boleta")
                    | (Q(rut_cliente__isnull=False) & Q(nombre_cliente__isnull=False))
                ),
                name="chk_boleta_campos",
            ),
            models.CheckConstraint(
                check=(
                    ~Q(tipo_documento="factura")
                    | (
                        Q(rut_empresa__isnull=False)
                        & Q(razon_social__isnull=False)
                        & Q(giro__isnull=False)
                    )
                ),
...
class MovimientoStock(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["fecha_movimiento"], name="idx_movimiento_stock_fecha"
            ),
...
class PedidoItem(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=Q(cantidad__gt=0), name="pedido_item_cantidad_check"
            ),
            models.CheckConstraint(
                check=Q(precio_unitario__gte=0),
                name="pedido_item_precio_unitario_check",
            ),
            models.CheckConstraint(
                check=Q(subtotal__gte=0), name="pedido_item_subtotal_check"
            ),
        ]
